/*
 * V 1.9 of Multisensor 6 code 11/4/2015
 ...
 * Bradley Shubin 10/28/2015 - 11/4/2015
 *
 ...
 */
 ...
 metadata {
	definition (name: "HaasAeonMulti6", namespace: "drandyhaas", author: "Bradley Shubin - Robert Vandervoort - Edits by Haas") {
    ...
    preferences {
		input ("tempoffset",
            ...
            description: "Only -10.0 through 10.0 is valid",
			defaultValue: 0,
            range: "-10..10",
            required: false,
		...
		input ("humidityoffset",
            ...
            description: "Only -50 through 50 is valid",
			defaultValue: 0,
            range: "-50..50",
			required: false,
            displayDuringSetup: false)
		input ("luminanceoffset",
            ...
            description: "Only -1000 through 1000 is valid",
			defaultValue: 0,
            range: "-1000..1000",
            required: false,
	        displayDuringSetup: false)
		input ("ultravioletoffset",
            ...
            description: "Only -10 through 10 is valid",
			defaultValue: 0,
            range: "-10..10",
	        required: false,
			displayDuringSetup: false)
    ...
    tiles {
		...
		details([
        	"motion","temperature","humidity","illuminance","tamper","ultravioletIndex","battery","configure"
            ])
	}
}
...
def zwaveEvent(physicalgraph.zwave.commands.sensormultilevelv5.SensorMultilevelReport cmd)
{
	...
	switch (cmd.sensorType) {
		case 1:
            ...
            //ACH
            def oldtemp = device.currentValue("temperature").toFloat()
            def tempdiff = (map.value.toFloat()-oldtemp).round(1)
            log.debug("new map temp value is $map.value and old temp was $oldtemp and tempdiff is $tempdiff ")
            if (tempdiff>10.0){
            	map.value=(oldtemp+5.0).round(1).toString()
                log.debug("now map temp value is $map.value")
            }
            if (tempdiff<-10.0){
            	map.value=(oldtemp-5.0).round(1).toString()
                log.debug("now map temp value is $map.value")
...
def configure() {
	...
	def PIRsens = PIRsensitivity
	def ReportingInt = ReportingInterval //.toInteger()
    def tempoff = (tempoffset*10)
    tempoff = Math.round(tempoff)
	def humidityoff = humidityoffset
	def luminanceoff = luminanceoffset
	def ultravioletoff = ultravioletoffset
