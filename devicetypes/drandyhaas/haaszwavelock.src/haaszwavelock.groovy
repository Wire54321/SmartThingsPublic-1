 ...
 */
metadata {
	definition (name: "HaasZwaveLock", namespace: "drandyhaas", author: "Andrew Haas") {
...
def poll() {
	...
	if (state.assoc != zwaveHubNodeId && secondsPast(state.associationQuery, 19 * 60)) {
		...
		// Only check lock state if it changed recently or we haven't had an update in 10 minutes
		def timesincelastpoll = (new Date().time) - state.lastPoll
        log.trace "$state.lastPoll $timesincelastpoll"
		if (!state.lastPoll || timesincelastpoll > 10*60*1000) {
        	log.trace "operationGet()"
		...
		} else if (!state.lastbatt || (new Date().time) - state.lastbatt > 24*60*60*1000) { //24 hours
